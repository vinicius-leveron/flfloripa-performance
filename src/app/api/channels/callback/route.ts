...
async function exchangeCodeForToken(platform: string, code: string, redirectUri: string) {
  ...
  if (!response.ok) {
    throw new AppError('EXTERNAL_API_ERROR', `Falha ao trocar código OAuth com ${platform}`, 502);
  }
...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.redirect(new URL('/settings/channels?error=oauth_denied', request.url));
    }

    if (!code || !stateParam) {
      throw new AppError('VALIDATION_ERROR', 'Código ou state ausente', 400);
    }
    ...
    await prisma.channel.upsert({
      where: {
        id: `${state.platform}-${state.userId}`,
      },
      create: {
        platform: state.platform,
        accountName: tokenData.name || state.platform.toLowerCase(),
